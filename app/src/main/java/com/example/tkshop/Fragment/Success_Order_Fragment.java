...
public class Success_Order_Fragment extends Fragment {

    RecyclerView recyclerView_Order_success;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View root = inflater.inflate(R.layout.fragment_success__order_, container, false);
        reference = FirebaseDatabase.getInstance().getReference("Request");
        requestReference = FirebaseDatabase.getInstance().getReference("Request").orderByChild("status").equalTo(4);
        recyclerView_Order_success = root.findViewById(R.id.recyclerView_Order_success);
        recyclerView_Order_success.setHasFixedSize(true);
        LinearLayoutManager manager = new LinearLayoutManager(getActivity(), LinearLayoutManager.VERTICAL, true);
        manager.setStackFromEnd(true);
        recyclerView_Order_success.setLayoutManager(manager);
        ...
        recyclerView_Order_success.setAdapter(adapter);
        return root;
    }
