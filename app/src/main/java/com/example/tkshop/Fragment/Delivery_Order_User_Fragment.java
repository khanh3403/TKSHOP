...
public class Delivery_Order_User_Fragment extends Fragment {
    RecyclerView recyclerView_delivery_order_user;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View root = inflater.inflate(R.layout.fragment_delivery__order__user_, container, false);
        reference = FirebaseDatabase.getInstance().getReference("Request");
        requestReference = FirebaseDatabase.getInstance().getReference("Request").orderByChild("phone_status").equalTo(Common.currentUser.getPhone()+"_"+3);
        recyclerView_delivery_order_user = root.findViewById(R.id.recyclerView_delivery_order_user);
        recyclerView_delivery_order_user.setHasFixedSize(true);
        LinearLayoutManager manager = new LinearLayoutManager(getActivity(), LinearLayoutManager.VERTICAL, true);
        manager.setStackFromEnd(true);
        recyclerView_delivery_order_user.setLayoutManager(manager);
        ...
        recyclerView_delivery_order_user.setAdapter(adapter);
        return root;
    }
}